...
if __name__ == '__main__':
    ...
    if record == 1:
        ...
        perf = np.array(perf)
        # np.save('result_visualization/figures/curve/perf.npy', perf)
        # np.save('result_visualization/figures/curve/ra.npy', np.array(model.ra_record))
        # np.save('result_visualization/figures/curve/loss.npy', np.array(model.loss_record))
        # np.save('result_visualization/figures/curve/beta.npy', np.array(model.beta_record))
        if not os.path.exists(opt.record_folder):                 # make sure logger path exists
            os.mkdir(opt.record_folder)
            
        path_name = opt.record_folder + '/perf.npy'
        np.save(path_name, perf)

        path_name = opt.record_folder + '/ra.npy'
        np.save(path_name, np.array(model.ra_record))

        path_name = opt.record_folder + '/loss.npy'
        np.save(path_name, np.array(model.loss_record))

        path_name = opt.record_folder + '/beta.npy'
        np.save(path_name, np.array(model.beta_record))
